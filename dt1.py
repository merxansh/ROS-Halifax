...
class SquareMoveOdom(SquareMove):
	...
	def turn_of(self, a, ang_speed=0.2):
		...
		while ( abs(self.wrap_angle(self.get_z_rotation(self.odom_pose.orientation) - a_init)) ) < a and not ros.is_shutdown():
			...
			if ang_speed > 0 and self.wrap_angle(self.get_z_rotation(self.odom_pose.orientation)-a_init) < -0.5 :
				break
			elif ang_speed < 0 and self.wrap_angle(a_init-self.get_z_rotation(self.odom_pose.orientation)) < -0.5 :
				break
